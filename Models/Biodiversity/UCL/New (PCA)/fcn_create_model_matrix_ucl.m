function model_matrix = fcn_create_model_matrix_ucl(data_cells, landuses, decade_string, pca_info)
    ...
    % - pca_info: a structure containing fields: (i) .means for centering
    %   the landuse data, and (ii) .eigenvectors for projecting the data.
    %   This has been added for the new models covering pollinator and
    %   priority species, which are regressions of principal components.
    % Outputs:
    ...
    % -------------------------------
    % Convert from hectares to percentage
    model_matrix_pct = model_matrix_ha / 4;
    
    % Center data using PCA means
    model_matrix_centred = model_matrix_pct - pca_info.means;
    
    % Project using PCA eigenvectors (1st 7 eigenvectors)
    model_matrix_pca = model_matrix_centred * pca_info.eigenvectors(:, 1:7);
    
    % Add intercept
    % -------------
    model_matrix = [intercept, model_matrix_pca];
end
